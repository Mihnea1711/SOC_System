...
def send_request(target_url, payload):
    ...
    try:
        ...
        print(f"[SQLi] Sent payload: {payload} | Status: {response.status_code} | Time: {elapsed:.2f}s")
        
    except requests.exceptions.RequestException as e:
        print(f"[SQLi] Request failed: {e}")

def simulate_sqli(target_url, count, delay):
...
if __name__ == "__main__":
    ...
    simulate_sqli(args.target, args.count, args.delay)
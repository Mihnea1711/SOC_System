...
* **Nginx (Monitored Host):** A mock web server serving as the target for our attack simulations.
* **Filebeat:** Tails and parses Nginx `access.log` and `error.log` files.
* **Packetbeat:** Sniffs raw network traffic (TCP/HTTP flows) on the Docker bridge network.
...
* **Enricher:** Adds context to the event, such as GeoIP location and Threat Intelligence (known malicious IPs).
